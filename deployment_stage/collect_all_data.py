...
def collect_all_data():
    ...
    if obs_normalize:
        agent_obs_mean_list, agent_obs_std_list, oppo_obs_mean_list, oppo_obs_std_list = cal_agent_oppo_obs_mean(
            offline_data, total=average_total_obs)
    ...
    # if obs_normalize:
    #     agent_obs_mean_list, agent_obs_std_list, oppo_obs_mean_list, oppo_obs_std_list = cal_agent_oppo_obs_mean(
    #         online_data, total=average_total_obs)
    #     CONFIG_DICT["AGENT_OBS_MEAN"] = agent_obs_mean_list
    #     CONFIG_DICT["AGENT_OBS_STD"] = agent_obs_std_list
    #     CONFIG_DICT["OPPO_OBS_MEAN"] = oppo_obs_mean_list
    #     CONFIG_DICT["OPPO_OBS_STD"] = oppo_obs_std_list
    ...
    encoder.load_model('model/MS-unseen-3oppo-ours-a1-l1-W5-K20-20240710014138/pel_encoder_iter_199', device=device)
    encoder.eval()
    ...
    if log_to_wandb:
        wandb.init(
            name=exp_prefix,
            group=group_name,
            project="TEST_online2" + f"-{test_mode}",
            config=CONFIG_DICT,
        )
    ...
    all_context_window = collect_all_episode(
        encoder=encoder,
        decoder=decoder,
    ...
    # with open('collected_data_target70.pkl', 'wb') as f:
    #     pickle.dump(all_online_data, f)
