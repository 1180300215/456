...
def load_oppo_data(data_path, oppo_index, act_dim, config_dict):
    with open(data_path, 'rb') as f:
        data = pickle.load(f)     # 从file中读取一个字符串，并将它重构为原来的python对象
        data_o = data["observations"]
        data_a = data["actions"]
        data_r = data["rewards"]
        data_o_next = data["next_observations"]
    num_oppo_policy = len(data_o)     # 对手策略的数量
    config_dict["NUM_OPPO_POLICY"] = num_oppo_policy
    data_list = [[] for _ in range(num_oppo_policy)]   # data_list = [ [] [] [] [] [] ]
    for i in range(num_oppo_policy):
        num_agent_policy = len(data_o[i])    # 受控代理策略的数量 对于 策略i
        for j in range(num_agent_policy):
            num_epis = len(data_o[i][j])   # i，j的 episode 的数量
            for e in range(num_epis):
                num_steps = len(data_o[i][j][e])   # i，j 在 episode e 的步骤数
    ...
    num_oppo_trajs_list = []          # 对手各个策略的轨迹数量列表
    for i in range(num_oppo_policy):
...
def get_batch(offline_data, config_dict):
    ...
    num_oppo_trajs_list = config_dict["NUM_OPPO_TRAJS"]    # 对手各个策略交互轨迹 列表
...
def get_batch_mix(offline_data,online_data,config_dict):
    ...
    online_mix_data = [oppo_data for oppo_data_list in online_data for oppo_data in oppo_data_list]  # 在线得到的数据
    ...
    shuffle_offline_index = np.arange(num_offline_total_trajs)  # 返回一个有终点和起点的固定步长的排列  所有的轨迹
    ...
    def fn(batch_size=batch_size, max_len=K):
        ...
        for k in range(int(batch_size/2)):
            ...
            a.append(traj['actions'][:max_len].reshape(1, -1, act_dim))
            r.append(traj['rewards'][:max_len].reshape(1, -1, 1))
            o_gen.append(traj_gen['observations'][:max_len].reshape(1, -1, obs_dim))  # 加入一个 1*100*8
            a_gen.append(traj_gen['actions'][:max_len].reshape(1, -1, act_dim))
            timesteps.append(
                np.arange(1, (n_o[-1].shape[1] + 1)).reshape(1, -1)
            )  # 1- 100
